...
const PieceToAll: React.FC<{ t: any }> = ({ t }) => {
  ...
  const calculate = () => {
    ...
    if (l > 0 && w > 0 && p > 0) {
      ...
      const calcResults = { murubba: totalMurubba, meter: totalMeter, priceMurubba: pMur, priceMeter: pMet };
      setResult(calcResults);

      saveToHistory({
        tabType: CalculatorTab.PIECE_TO_ALL,
        inputs: {
          [t.length]: length,
          [t.width]: width,
          [t.pieces]: pieces
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h2 className="text-xl font-bold text-[#1e293b]">{t.tabs.piece}</h2>
      </div>
      ...
      <div className="app-card bg-white p-6 border border-slate-100">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            <label className="text-[#d91e4a] text-xs font-bold">{t.pieces}</label>
            <input
              type="number"
              value={pieces}
              onChange={(e) => setPieces(e.target.value)}
              className="input-box w-full p-4 outline-none border-[#d91e4a]/20 focus:ring-1 focus:ring-red-400"
            />
          </div>
          <div className="space-y-1">
        ...
        <button
          onClick={calculate}
          className="w-full mt-6 bg-[#d91e4a] text-white py-5 rounded-2xl font-bold text-lg flex items-center justify-center gap-2 shadow-xl shadow-red-100 active:scale-95 transition-all"
        >
          {t.calc} →
        </button>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#00a651] text-white p-6 rounded-3xl text-center shadow-lg shadow-green-100">
              <p className="text-[10px] font-bold uppercase mb-2 opacity-80 tracking-widest">{t.totalMur}</p>
